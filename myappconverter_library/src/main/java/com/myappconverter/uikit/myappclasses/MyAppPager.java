...
public class MyAppPager extends HorizontalScrollView {
    LinearLayout contents;
    private LinkedList<MyAppOnPageChangeListener> listeners;

    public MyAppPager(Context ctx, AttributeSet attrs) {
        super(ctx, attrs);
        contents = new LinearLayout(ctx);
        contents.setLayoutParams(new LayoutParams(LayoutParams.WRAP_CONTENT,
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (int i = 0; i < contents.getChildCount(); i++) {
            ...
            if (child.getLayoutParams().width != specSize) {
                child.setLayoutParams(new LinearLayout.LayoutParams(specSize,
                        LayoutParams.FILL_PARENT));
            }
        }
    ...
    public void onScrollChanged(int l, int t, int oldl, int oldt) {
        if (listeners != null) {
            for (MyAppOnPageChangeListener list : listeners) {
    ...
    public void addOnPageChangeListener(
        MyAppOnPageChangeListener onPageChangeListener) {
        if (listeners == null) {
            listeners = new LinkedList<MyAppOnPageChangeListener>();
        }

        listeners.add(onPageChangeListener);
    }

    public boolean removeOnPageChangeListener(MyAppOnPageChangeListener l) {
        if (listeners != null) {
